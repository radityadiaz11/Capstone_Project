...
const DataNilai_Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/academic/scores');
  ...
  const statCards = [
    { label: 'Di atas KKM', value: data.stats?.diAtasKKM || 0, sub: 'siswa', subColor: '#10b981', accent: '#10b981' },
    { label: 'Di bawah KKM', value: data.stats?.diBawahKKM || 0, sub: 'Perlu remedial', subColor: '#ef4444', accent: '#ef4444' },
    { label: 'Nilai tertinggi', value: data.stats?.tertinggi?.val || 0, sub: data.stats?.tertinggi?.name || '-', subColor: '#555', accent: '#1a56db' },
    { label: 'Nilai terendah', value: data.stats?.terendah?.val || 0, sub: data.stats?.terendah?.name || '-', subColor: '#555', accent: '#ea580c' },
  ];
