...
function StatistikSNBP_Page() {
    ...
    // ── Data Statistik ────────────────────────────────────────────────
    const totalSiswa = dbData ? dbData.overview.totalSiswa : 216;
    const totalSiap = dbData ? dbData.overview.siap : 142;
    const totalBerisiko = dbData ? dbData.overview.berisiko : 31;
    const totalPerhatian = dbData ? dbData.overview.perhatian : 43;

    const statCards = [
        { label: 'Total Siswa Kelas XII', value: totalSiswa.toString(), sub: '6 kelas paralel', color: 'blue' },
        { label: 'On Track', value: totalSiap.toString(), sub: `${totalSiswa ? Math.round((totalSiap/totalSiswa)*100) : 0}% dari total`, color: 'green' },
        { label: 'Siswa Berisiko', value: totalBerisiko.toString(), sub: `${totalSiswa ? Math.round((totalBerisiko/totalSiswa)*100) : 0}% dari total`, color: 'red' },
    ];

    const pctSiap = totalSiswa ? ((totalSiap/totalSiswa)*100).toFixed(1) : 0;
    const pctPerhatian = totalSiswa ? ((totalPerhatian/totalSiswa)*100).toFixed(1) : 0;
    const pctBerisiko = totalSiswa ? ((totalBerisiko/totalSiswa)*100).toFixed(1) : 0;

    const distribusiData = [
        { label: 'Siap (>70%)', count: totalSiap, pct: pctSiap, color: '#16a34a' },
        { label: 'Perhatian (50–70%)', count: totalPerhatian, pct: pctPerhatian, color: '#f59e0b' },
        { label: 'Berisiko (<50%)', count: totalBerisiko, pct: pctBerisiko, color: '#dc2626' },
    ];

    const kelasPerforma = dbData ? dbData.performaKelas : [
                                    ...
                                    <div className="snbs-donut-wrapper">
                                        <div className="snbs-donut">
                                            <div className="snbs-donut-center">
                                                <span className="snbs-donut-total">{totalSiswa}</span>
